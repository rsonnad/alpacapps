...
-- Migrates 7 Govee H601F bars from Govee Cloud API to HAOS Govee integration
-- Creates HAOS groups: outhouse_ceiling (5 — 4 main + 1 changing), outhouse_stalls (2), outhouse_all (7)
...
VALUES
  ...
  ('Outhouse Changing',   'Outhouse', 7, 'light.outhouse_changing',   'Govee', 'H601F', 'wifi_govee', NULL,                       'H601F', 'light_bar', true);
...
SET backend = 'home_assistant',
    ...
    metadata = '{"entity_ceiling":"light.outhouse_ceiling","entity_stalls":"light.outhouse_stalls","entity_changing":"light.outhouse_changing","device_count":7,"sku_individual":"H601F"}'::jsonb,
    updated_at = now()
...
-- light.outhouse_ceiling  (unique_id: outhouse_ceiling_lights)  → outhousemain1-4 + outhouse_changing
-- light.outhouse_stalls   (unique_id: outhouse_stalls_lights)   → outhouse_stall_left, outhouse_stall_right
-- light.outhouse_all      (unique_id: outhouse_all_lights)      → all 7 bars
