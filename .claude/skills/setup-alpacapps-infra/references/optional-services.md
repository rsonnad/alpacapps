...
## Open Brain (Shared AI Memory)

### Ask User

> Open Brain gives all your AI tools a shared memory. Thoughts captured from Slack (or any MCP client) are stored as vector embeddings and searchable by any connected AI.
>
> 1. Sign up at https://openrouter.ai/ and create an API key (used for embeddings + metadata extraction)
> 2. **Optional — Slack capture:** Create a Slack app at https://api.slack.com/apps
>    - OAuth Scopes: `channels:history`, `groups:history`, `chat:write`
>    - You'll set the Event Subscription URL after deployment
> 3. Paste: **OpenRouter API key** (and optionally **Slack Bot Token** + **Capture Channel ID**)
...
CREATE POLICY "Service role only" ON thoughts FOR ALL
...
1. Set secrets:
   ...
   # If Slack capture enabled:
   supabase secrets set SLACK_BOT_TOKEN={token} SLACK_CAPTURE_CHANNEL={channel_id}
   ```
2. Generate MCP access key:
   ```bash
   ACCESS_KEY=$(openssl rand -hex 32)
   echo "MCP access key: $ACCESS_KEY"
   supabase secrets set MCP_ACCESS_KEY=$ACCESS_KEY
   ```
3. Create and deploy `ingest-thought` edge function with `--no-verify-jwt`:
   - Receives Slack webhook events or direct POST
   - Generates embedding via OpenRouter (`openai/text-embedding-3-small`)
   - Extracts metadata via OpenRouter (`openai/gpt-4o-mini`): type, tags, people, action items, priority
   - Inserts into `thoughts` table
4. Create and deploy `open-brain-mcp` edge function with `--no-verify-jwt`:
   - MCP server (Hono + @modelcontextprotocol/sdk)
   - Tools: `search_thoughts`, `browse_recent`, `stats`, `capture_thought`
   - Auth: validates `x-brain-key` header or `?key=` URL param against `MCP_ACCESS_KEY`
5. If Slack capture enabled:
   - Tell user to set Event Subscription URL: `https://{REF}.supabase.co/functions/v1/ingest-thought`
   - Subscribe to events: `message.channels`, `message.groups`
   - Install app to workspace, invite bot to capture channel
6. Connect Claude Code:
   ```bash
   claude mcp add --transport http open-brain \
     "https://{REF}.supabase.co/functions/v1/open-brain-mcp" \
     --header "x-brain-key: {ACCESS_KEY}"
   ```
7. Test: capture a thought via direct POST, then search via MCP
8. Append to `docs/CREDENTIALS.md`: OpenRouter key, MCP access key, Slack tokens
9. Append to `docs/INTEGRATIONS.md`: Open Brain config, MCP URL, connected clients
10. Append to `docs/SCHEMA.md`: thoughts table, match_thoughts function
