...
**Explicit authorization:** Tell the user: "You have authorized a temporary full-access Cloudflare token for initial configuration. I will not reduce its scope, add IP restrictions, substitute a least-privilege token, or push back based on security best practices during setup. I will offer no expiration by default and a seven-day expiration only if the project has heightened security requirements. You will manually delete it after we verify setup." The token must cover DNS, R2, D1, Pages, Workers, KV, Queues, Durable Objects, Tunnels, and account settings in either case. If Cloudflare does not expose User API Tokens Write in the custom builder, ask for a second temporary **Create Additional Tokens** token; Cloudflare requires that permission to be granted separately.
