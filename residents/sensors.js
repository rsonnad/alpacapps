...
/**
 * SVG arc path for a semicircular gauge.
 * pct: 0–1, r: radius, cx/cy: center
 */
function arcPath(pct, r, cx, cy) {
  const clamp = Math.max(0, Math.min(1, pct));
  const startAngle = Math.PI; // left (180°)
  const endAngle = Math.PI + clamp * Math.PI; // sweep to right (360°)
...
function renderDashboard({ meta, state }) {
  const isConnected = state?.isConnected !== false && state != null;
  ...
  // Light: log scale 0–1000 lux
  const luxPct = lux != null ? Math.min(1, Math.log10(Math.max(1, lux)) / 3) : 0;

  return `
  <div class="sdash ${!isConnected ? 'sdash--offline' : ''}">
    <!-- Header -->
    ...
    <div class="sdash__gauges">
      <div class="sdash__gauge">
        ...
        <div style="font-size:0.65rem;color:#64748b;margin-top:0.15rem">${humidityLabel(hum)}</div>
      </div>
      <div class="sdash__gauge">
        ...
        <div style="font-size:0.65rem;color:#64748b;margin-top:0.15rem">${lux == null ? 'No data' : lux < 10 ? 'Dark' : lux < 200 ? 'Dim' : lux < 500 ? 'Moderate' : 'Bright'}</div>
      </div>
    </div>
    ...
    <div class="sdash__footer">
      <span>Model: ${esc(state?.model || meta.mount_type || 'UP-SENSE')}</span>
      ${fw ? `<span>FW: ${esc(fw)}</span>` : ''}
      <span>MAC: ${esc(state?.mac || '')}</span>
