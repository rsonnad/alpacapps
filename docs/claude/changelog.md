...
40. **Brand Style Guide & Email Consistency** - `brand_config` DB table (singleton JSONB) stores all brand tokens. `shared/brand-config.js` client loader. `email-brand-wrapper.ts` wraps all emails in branded shell (header with logo+wordmark, body, footer). Admin style guide page at `spaces/admin/brand.html`. Colors: cream/amber/dark palette. Font: DM Sans. Logos: dark/light variants of alpaca icon + wordmark.