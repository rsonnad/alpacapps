...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (tokenError || !tokenRecord) {
      return errorPage('Invalid Link', 'This link is not valid. Please contact the Alpaca Playhouse team for a new one.');
    }

    if (new Date(tokenRecord.expires_at) < new Date()) {
      return errorPage('Link Expired', 'This link has expired. Please contact the Alpaca Playhouse team for a new one.');
    }
    ...
    const baseUrl = 'https://alpacaplayhouse.com/associates/worktracking.html';
