 ...
 * 1. Sender identity: "PAI at the Alpaca Playhouse"
...
serve(async (_req) => {
  ...
  const expectedSenderName = "PAI at the Alpaca Playhouse";
  for (const [key, val] of Object.entries(SENDER_MAP)) {
    ...
    if (val.reply_to !== "pai@alpacaplayhouse.com") {
      violations.push({
        rule: 1,
        ruleName: "Reply-To",
        details: `SENDER_MAP["${key}"].reply_to = "${val.reply_to}" — expected "pai@alpacaplayhouse.com"`,
      });
    }
  }
  ...
  const stalePatterns = [
    "Best regards,",
    "Yours,<br>The Alpaca Playhouse Community Team",
    "Thanks,<br>Alpaca Playhouse",
  ];
